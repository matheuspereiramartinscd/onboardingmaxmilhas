...
function extractQuestionsAndOptions(courses) {
    let content = '';

    courses.forEach(course => {
        content += `Curso: ${course.title}\n\n`;
        course.questions.forEach((q, index) => {
            content += `Pergunta ${index + 1}: ${q.question}\n`;
            q.options.forEach((option, i) => {
                content += `  Opção ${i + 1}: ${option}\n`;
            });
            // Adiciona a resposta correta logo após as opções
            content += `Resposta correta: ${q.correctAnswer}\n\n`;
        });
        content += '\n';
    });

    fs.writeFileSync('questions_and_options.txt', content, 'utf8');
    console.log("Arquivo 'questions_and_options.txt' criado com sucesso.");
}

// Exemplo de chamada da função com o array `courses`
extractQuestionsAndOptions(courses);